...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  // Temporary Ads/Ticker state - initialized safely
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (user) {
      setCurrentUser(user);
      setAuthor(user.name);
      setActiveTab('news'); // Sigurohemi që hapet te lajmet
    } else {
  ...
  // Login View (Nëse nuk është kyçur asnjë përdorues)
  if (!currentUser) {
    return (
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50">
          {activeTab === 'news' && (
            <div className="flex flex-col lg:flex-row min-h-full">
                ...
                <form onSubmit={handleSubmitNews} className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <input value={author} onChange={(e) => setAuthor(e.target.value)} className="w-full bg-gray-50 p-4 rounded-2xl font-bold text-xs outline-none border-2 border-transparent focus:border-red-600" placeholder="Redaksia" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase text-gray-400 ml-2">Teksti i lajmit</label>
                    <textarea value={excerpt} onChange={(e) => setExcerpt(e.target.value)} required rows={8} className="w-full bg-gray-50 p-4 rounded-2xl text-sm outline-none border-2 border-transparent focus:border-red-600 leading-relaxed" placeholder="Përmbajtja..."></textarea>
                  </div>

                  <div className="flex gap-4 pt-4">
                    <button type="submit" className="flex-1 bg-gray-900 text-white py-5 rounded-2xl font-black uppercase text-[11px] tracking-widest hover:bg-red-600 transition-all shadow-lg">
                      {editingId ? 'Ruaj Ndryshimet' : 'Publiko Tani'}
                    </button>
                    {editingId && (
              ...
              {/* Right Column: List */}
              <div className="p-8 lg:p-14 lg:w-1/2 bg-gray-50">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                  <h3 className="font-black uppercase text-[11px] text-gray-400 tracking-widest">Lajmet e fundit</h3>
                ...
                <div className="space-y-4">
                  {filteredNewsInList.length > 0 ? (
                    filteredNewsInList.map(n => (
                      <div key={n.id} className="bg-white p-4 rounded-2xl flex justify-between items-center shadow-sm border border-transparent hover:border-red-600 transition-all group">
                        <div className="flex items-center gap-4 min-w-0">
                          <img src={n.image} className="w-12 h-12 rounded-xl object-cover shadow-sm" alt="news" />
                          <div className="min-w-0">
                            <h4 className="text-[11px] font-black truncate max-w-[150px] sm:max-w-[250px]">{n.title}</h4>
                            <div className="flex gap-2 items-center mt-1">
                              <span className="text-[8px] font-black uppercase text-red-600 bg-red-50 px-2 py-0.5 rounded">{n.category}</span>
                              <span className="text-[8px] font-bold text-gray-400 uppercase tracking-tighter">@{n.author}</span>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200 opacity-40 font-black uppercase text-[10px] tracking-widest">Nuk u gjet asnjë lajm</div>
                  )}
                </div>
              </div>
            </div>
          ...
          {/* Admin Only Tabs with extra protection */}
             ...
             <div className="p-8 lg:p-14">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {contactRequests?.map(req => (
                    <div key={req.id} className="bg-white p-6 rounded-[2rem] border-2 border-gray-50 shadow-sm">
                       <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center gap-3">
                             <div className="w-10 h-10 bg-red-50 text-red-600 rounded-full flex items-center justify-center text-xs font-black">{req.name?.charAt(0)}</div>
                             <div>
                                <h4 className="font-black text-sm">{req.name}</h4>
                                <p className="text-[9px] font-bold text-gray-400 uppercase">{req.date}</p>
                             </div>
                          </div>
                          <span className={`text-[8px] font-black uppercase px-3 py-1 rounded-full ${req.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-yellow-50 text-yellow-600'}`}>{req.status}</span>
                       </div>
                       <p className="text-[11px] text-gray-600 mb-4 bg-gray-50 p-4 rounded-xl italic">"{req.message}"</p>
                       <div className="flex justify-between items-center pt-2">
                          <div className="text-[10px] font-bold text-gray-400">{req.phone}</div>
                          <div className="flex gap-2">
                             {req.status === 'pending' && <button onClick={() => onUpdateRequestStatus(req.id, 'approved')} className="text-[9px] font-black uppercase text-green-600 px-3 py-1">Aprovo</button>}
                             <button onClick={() => onDeleteRequest(req.id)} className="text-[9px] font-black uppercase text-red-600 px-3 py-1">Fshij</button>
                          </div>
                       </div>
                    </div>
                  ))}
          ...
          {activeTab === 'users' && currentUser.role === 'Admin' && (
            <div className="p-8 lg:p-14 flex flex-col lg:flex-row gap-10">
               <div className="lg:w-1/3 bg-white p-8 rounded-[2rem] shadow-sm">
                  <h3 className="font-black uppercase text-sm mb-6">Regjistro Staf</h3>
                  <form onSubmit={(e) => {
                    e.preventDefault();
                    onAddUser({
                    ...
                    setNewUserName(''); setNewUserUsername(''); setNewUserPassword('');
                     ...
                     <input value={newUserName} onChange={e => setNewUserName(e.target.value)} required className="w-full bg-gray-50 p-4 rounded-2xl outline-none text-xs font-bold" placeholder="Emri i plotë" />
                     <input value={newUserUsername} onChange={e => setNewUserUsername(e.target.value)} required className="w-full bg-gray-50 p-4 rounded-2xl outline-none text-xs font-bold" placeholder="Username" />
                     <input value={newUserPassword} onChange={e => setNewUserPassword(e.target.value)} required type="password" className="w-full bg-gray-50 p-4 rounded-2xl outline-none text-xs font-bold" placeholder="Password" />
                     <select value={newUserRole} onChange={e => setNewUserRole(e.target.value as UserRole)} className="w-full bg-gray-50 p-4 rounded-2xl outline-none text-[10px] font-black uppercase">
                        <option value="Moderator">Moderator</option>
                        <option value="Admin">Admin</option>
                     </select>
                     <button type="submit" className="w-full bg-red-600 text-white py-4 rounded-2xl font-black uppercase text-[10px]">Krijo Llogarinë</button>
                  </form>
               </div>
               <div className="lg:w-2/3 space-y-4">
                  <h3 className="font-black uppercase text-sm mb-6">Stafi Aktive</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                     {users?.map(u => (
                        <div key={u.id} className="bg-white p-6 rounded-[2rem] flex justify-between items-center shadow-sm">
                           <div>
          ...
          {activeTab === 'ads' && currentUser.role === 'Admin' && (
             <div className="p-8 lg:p-14 max-w-2xl mx-auto space-y-10">
                <div className="bg-red-50 p-8 rounded-[2.5rem] border border-red-100">
                   <h4 className="text-[10px] font-black uppercase text-red-600 mb-6 tracking-widest">Shiriti i Lajmeve</h4>
                   <div className="flex gap-4">
                      ...
                      <button onClick={() => { onUpdateTicker(tempTicker); alert('U ruajt!'); }} className="bg-red-600 text-white px-8 rounded-2xl font-black uppercase text-[10px]">Ruaj</button>
                   </div>
                </div>
                <div className="bg-blue-50 p-8 rounded-[2.5rem] border border-blue-100 space-y-4">
                   <h4 className="text-[10px] font-black uppercase text-blue-600 mb-2 tracking-widest">Marketing Header</h4>
                   <input value={tempAd.title} onChange={e => setTempAd({...tempAd, title: e.target.value})} className="w-full bg-white p-4 rounded-2xl text-xs font-bold outline-none" placeholder="Titulli reklamës" />
                   <input value={tempAd.imageUrl} onChange={e => setTempAd({...tempAd, imageUrl: e.target.value})} className="w-full bg-white p-4 rounded-2xl text-[10px] outline-none" placeholder="URL Foto" />
                   <input value={tempAd.linkUrl} onChange={e => setTempAd({...tempAd, linkUrl: e.target.value})} className="w-full bg-white p-4 rounded-2xl text-[10px] outline-none" placeholder="URL Link" />
                   <button onClick={() => { onUpdateAd(tempAd); alert('U ruajt!'); }} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase text-[10px]">Përditëso Reklamën</button>
                </div>
             </div>
          )}
        </div>
